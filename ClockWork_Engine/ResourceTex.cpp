...
uint ResourceTex::SaveMeta(JSON& base_object, uint last_modification)
{
	...
	TextureImportingOptions importingOptions = App->res->textureImportingOptions;

	base_object.AddInt("texture_wrap", (int)App->res->textureImportingOptions.textureWrap);
	base_object.AddInt("texture_filtering", (int)App->res->textureImportingOptions.textureFiltering);
...
void ResourceTex::Load(JSON& base_object)
{
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	glGenTextures(1, &_gpu_ID);
	glBindTexture(GL_TEXTURE_2D, _gpu_ID);
