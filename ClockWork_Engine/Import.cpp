...
Import::Import() : Window(), _fileToImport(nullptr), _currentResourceType(ResourceType::RESOURCE_UNKNOWN)
{}
...
void Import::Enable(const char* file, ResourceType resourceType)
{
	_fileToImport = file;
...
bool Import::DrawModelImportingWindow()
{
	bool ret = true;
	...
	if (ImGui::Begin("Import File", NULL))
	{
		...
		{
			App->res->modelImportingOptions = _modelImportingOptions;
			App->res->ImportFile(_fileToImport);
			ret = false;
		}
		ImGui::SameLine();
		...
		if (ImGui::Button("Reset"))
			_modelImportingOptions = Options();
	}
	ImGui::End();
...
bool Import::DrawTextureImportingWindow()
{
	bool ret = true;
	...
	if (ImGui::Begin("Import File", NULL))
	{
		...
		if (ImGui::Button("OK", ImVec2(40, 20)))
		{
			App->res->textureImportingOptions = _textureImportingOptions;
			App->res->ImportFile(_fileToImport);
			ret = false;
		}
		ImGui::SameLine();
